...
#fonction void distribute automatically ipv6 address 
def distribute():
	i=1
	while i <= len(links):
		
		original_string = "2001:100:100:a::"
	...
	with open("i"+router['name'] +"_startup-config" + ".cfg", "w") as f:
		...
		for interface in router['interfaces']:
			if interface['name'] == "Loopback0":
				...
				f.write(f" ipv6 enable\n")
				##need a fonction to check if it contained rip or ispf
				RipOspf(router['name'],interface['name'])
				f.write(f"!\n")
				f.write(f"!\n")
			else:
